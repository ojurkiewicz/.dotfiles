...
if not vim.loop.fs_stat(lazypath) then
  vim.fn.system {
    ...
    'clone',
    '--filter=blob:none',
u   'https://github.com/folke/lazy.nvim.git',
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      local go_config = { dap_go_enabled = true }
...
vim.keymap.set('n', '<leader>sf',
  function() require('telescope.builtin').find_files({ hidden = true, find_command = { 'rg', '--files', '--no-ignore', '-g', '!.git', '-g', '!node_modules', '-g', '!venv', '-g', '!__pycache__' } }) end,
  { desc = '[S]earch [F]iles' })
